'''
This is the main script to run OpenAi's Acrobot-v1 environment
'''
...
from utils.Helpers import calc_moving_average, plot_rewards, play_acrobot
import matplotlib
import matplotlib.pyplot as plt

def main():
    parser = argparse.ArgumentParser(description="train and play an Acrobot v-1 agent")
    # modes
    ...
    parser.add_argument("-m", "--mode", type=str,
                        help="model's mode or state representation ('frame_diff', 'frame_seq'), default: 'frame_diff'")
    parser.add_argument("-e", "--episodes", type=int,
                        help="number of episodes to play or train, default: 5 (play), 5000 (train)")
    # for training
    ...
    parser.add_argument("-d", "--decay_rate", type=int,
                        help="number of episodes for epsilon decaying, default: 2000")
    ...
    if (args.train):
        if (args.name):
            ...
            name = None
        if (args.mode):
            if (args.mode == 'frame_diff') or (args.mode == 'frame_seq'):
        ...
        gym.logger.set_level(40) # Disable dtype warning
        ...
        if name is not None:
            ...
            agent = AcrobotAgent(env,
                            ...
                            use_dropout=dropout,
                            dropout_rate=dropout_rate)
            if obs_represent == 'frame_seq':
                agent.load_agent_state(path='./acrobot_agent_ckpt/acrobot_agent_frame_seq_rep.pth', copy_to_target_network=True, load_optimizer=True)
            else:
                agent.load_agent_state(path='./acrobot_agent_ckpt/acrobot_agent_frame_diff.pth', copy_to_target_network=True, load_optimizer=True)
        LOG_EVERY_N_STEPS = 5000
        training_status_path = './acrobot_agent_ckpt/' + agent.name + '_training.status'
    ...
    # Playing
        ...
        if name is not None:
            agent = AcrobotAgent(env, name=name, obs_represent=obs_represent)
        else:
            name = 'user_' + datetime.utcnow().strftime("%Y%m%d%H%M%S")
            if obs_represent == 'frame_seq':
                agent = AcrobotAgent(env, name=name, obs_represent=obs_represent)
                ...
                agent = AcrobotAgent(env, name=name, obs_represent=obs_represent, use_batch_norm=True)
                agent.load_agent_state(path='./acrobot_agent_ckpt/acrobot_agent_frame_diff.pth', copy_to_target_network=True, load_optimizer=True)
        # PLAY
        play_acrobot(env, agent, num_episodes)
    else:
